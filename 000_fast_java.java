...
---
 Сборщик мусора
1. подсчет ссылок - умный указатель, медленно, не используется
2. остановиться и скопировать - каждый раз останавливает программу, копирует используемые объекты в чистую память (дефрагментирует) и удаляет мусор; медленно
3. пометить и убрать - очистка вконце сканирования
---
