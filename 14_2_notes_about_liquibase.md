...
В `preConditions` идет проверка нужно ли выполнять changeSet. Например нужно ли удалять sequence если он существует. Или проверка выполнялся ли этот changeSet на стэнде в продакшене через `changeSetExecuted` (указываем тут старый файл). В `onFail` задается что делать при ошибки, **halt** (default) - остановится, **MARK_RAN** - продолжить, **warn** - вывести предупреждение.
```xml
