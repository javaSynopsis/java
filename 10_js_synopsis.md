...
* [Airbnb JavaScript Style Guide() {](https://github.com/airbnb/javascriptы) - примеры плохого и хорошего кода, **решение некоторых проблем**
...
2) паттерны символов различных алфовитов в js т.к. обычное `\w` совпадает только с латинским алфавитом https://stackoverflow.com/a/37668315

3) генератор unicode наборов символов для regexp чтобы не искать в табл https://apps.timwhitlock.info/js/regex#
...
#### Взять sub property объекта через
```js
const o1 = {
  a : 'b',
  c: {
...
#### Взять свойство указанного и переименовать его
Возьмет до двоеточие объекта свойство по имени и переименует его в то имя, что после двуеточие.
```js
const o1 = {
  a : 'b',
  c: {
...
27) в js часто может понадобится техника запуска скрипта вроде
```js
